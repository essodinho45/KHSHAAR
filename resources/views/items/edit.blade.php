                ...
                <div class="overflow-hidden overflow-x-auto border-b border-gray-200 bg-white dark:border-gray-900 dark:bg-gray-800 p-6">
                    <form action="{{ route('items.update', $item) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mt-2">
                            ...
                            <x-text-input id="description" name="description" value="{{ old('description', $item->description) }}" type="text" class="block mt-1 w-full" />
                            <x-input-error :messages="$errors->get('description')" class="mt-2" />
                        </div>
                        <div class="mt-2">
                            <x-input-label for="sub_category_id" :value="__('sub_category')" />
                            <select id="sub_category_id" name="sub_category_id" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block w-full">
                                @foreach ($subCategories as $subCategory)
                                    <option value="{{ $subCategory->id }}" @selected($item->sub_category_id == $subCategory->id)>{{ $subCategory->name }}</option>
                                @endforeach
                            </select>
